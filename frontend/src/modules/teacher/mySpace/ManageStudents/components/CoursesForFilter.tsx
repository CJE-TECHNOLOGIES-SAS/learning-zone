...
export default function CoursesForFilter({onToggleCoursesForFilter, onToggleOpcFilter}:Props) {
    ...
    const selectCourse =async(id:TCourse['id'])=>{
      loadInfoStudentRegisterByCourse(id)
      onToggleCoursesForFilter()
      onToggleOpcFilter()
    ...
    <div className='container-courses-filter'>
      <button className='btn-close-container-filter-courses' onClick={onToggleCoursesForFilter}>{<TbXboxX/>}</button>
        ...
        {coursesFilter?.map((courseFilter)=>(
            <div key={courseFilter.id} className='container-course-filter' onClick={()=>selectCourse(courseFilter.id)}>
